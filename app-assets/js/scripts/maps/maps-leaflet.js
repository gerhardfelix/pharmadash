...
$(function () {
  ...
  var basicMap = $('#basic-map'),
    circlePolygonMap = $('#circle-polygon-map'),
    draggableMarkerMap = $('#draggable-marker-map'),
    userLocationMap = $('#user-location-map'),
    customIconsMap = $('#custom-icons-map'),
    geoJsonMap = $('#geojson-map'),
    layerControlMap = $('#layer-control-map'),
    statesData = {
  ...
  // Basic Map
  if (basicMap.length) {
    var Map = L.map('basic-map').setView([42.35, -71.08], 10);
  ...
  // Marker Circle & Polygon Map
  if (circlePolygonMap.length) {
    var markerMap = L.map('circle-polygon-map').setView([51.5, -0.09], 12);
  ...
  if (draggableMarkerMap.length) {
    var draggableMap = L.map('draggable-marker-map').setView([48.817152, 2.455], 12);
  ...
  // User Location Map
  if (userLocationMap.length) {
    var userMap = L.map('user-location-map').setView([42.35, -71.08], 10);
    userMap.locate({
      setView: true,
      maxZoom: 16
    });
  ...
  if (customIconsMap.length) {
    var customIcons = L.map('custom-icons-map').setView([51.5, -0.09], 10);
    var greenLeaf = L.icon({
      iconUrl: '../app-assets/images/maps/leaf-green.png',
      shadowUrl: '../app-assets/images/maps/leaf-shadow.png',
      iconSize: [38, 95],
      shadowSize: [50, 64],
      iconAnchor: [22, 94],
      shadowAnchor: [4, 62],
    ...
    var redLeaf = L.icon({
      iconUrl: '../app-assets/images/maps/leaf-red.png',
      shadowUrl: '../app-assets/images/maps/leaf-shadow.png',
      iconSize: [38, 95],
      shadowSize: [50, 64],
      iconAnchor: [22, 94],
    ...
    var orangeLeaf = L.icon({
      iconUrl: '../app-assets/images/maps/leaf-orange.png',
      shadowUrl: '../app-assets/images/maps/leaf-shadow.png',
      iconSize: [38, 95],
      shadowSize: [50, 64],
      iconAnchor: [22, 94],
  ...
  // GeoJson Map
  if (geoJsonMap.length) {
    var geoMap = L.map('geojson-map').setView([44.2669, -72.576], 3);
  ...
  // Layer Control Map
  if (layerControlMap.length) {
    var kalbe = L.marker([-6.3212382, 107.1165034]).bindPopup('<a href="fv-kf.php" class="speciallink">PT Kalbe Farma, Tbk.</a>'),
      hexpharm = L.marker([-6.3199984, 107.1201646]).bindPopup('<a href="fv-hj.php" class="speciallink">PT Hexpharm Jaya</a>'),
      fima = L.marker([-6.2154818, 106.9728067]).bindPopup('<a href="fv-fm.php" class="speciallink">PT Finusolprima Farma Internasional</a>'),
      dankos = L.marker([-6.2064851, 106.9094691]).bindPopup('<a href="fv-df.php" class="speciallink">PT Dankos Farma</a>'),
      gof = L.marker([-6.2066078, 106.9102791]).bindPopup('<a href="fv-gof.php" class="speciallink">PT Global Onkolab Farma</a>');
    var cities = L.layerGroup([kalbe, hexpharm, fima, dankos, gof]);
